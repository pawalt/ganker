...
- Multi-node Qwen SFT lives in `modal_apps/qwen_sft_multinode/`. It uses Modal clustered functions plus `torchrun`, not the Monarch/i6pn role mesh. Cluster size is fixed at import time with `GANKER_QWEN_SFT_MULTINODE_NODES`. DP-only Qwen3 0.6B LoRA works with `GANKER_QWEN_SFT_MULTINODE_GPU=H100:8`; TP and PP model parallelism are supported in the Qwen LoRA smoke path. PP requires `grad_accum_steps >= pipeline_model_parallel_size`, so the default clean PP example uses `TP=2`, `PP=2`, `DP=2`, and `global_batch_size=4` on one `H100:8` clustered node. Run light clustered checks with `source ~/.codex/modal.env` and `GANKER_QWEN_SFT_MULTINODE_NODES=2 uv run modal run modal_apps/qwen_sft_multinode/sft.py --mode torchrun-env`, then `--mode nccl-smoke`, then `--mode qwen-lora-sft --max-steps 1 --sequence-length 32`. The clean TP example is `modal_apps/qwen_model_parallel_sft/sft.py`; run it with `GANKER_QWEN_SFT_MULTINODE_NODES=1 GANKER_QWEN_SFT_MULTINODE_GPU=H100:8 uv run modal run modal_apps/qwen_model_parallel_sft/sft.py`. The clean PP example is `modal_apps/qwen_pipeline_parallel_sft/sft.py`; run it with `GANKER_QWEN_SFT_MULTINODE_NODES=1 GANKER_QWEN_SFT_MULTINODE_GPU=H100:8 uv run modal run modal_apps/qwen_pipeline_parallel_sft/sft.py`. The large-model TP+PP example is `modal_apps/qwen_large_model_parallel_sft/sft.py`; it defaults to `Qwen/Qwen3-32B`, requires `TP=8`, `PP=2`, and should be launched with `GANKER_QWEN_SFT_MULTINODE_NODES=2 GANKER_QWEN_SFT_MULTINODE_GPU=H100:8 uv run modal run modal_apps/qwen_large_model_parallel_sft/sft.py`. Run the longer comparison with `GANKER_QWEN_SFT_MULTINODE_NODES=2 uv run modal run modal_apps/qwen_sft_multinode/compare_hf.py --dataset-size 256 --max-steps 20 --sequence-length 256`; it compares multinode Ganker/Megatron, HF Trainer DDP, and a single-node Ganker/Megatron baseline from the same Modal app.
