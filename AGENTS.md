...
- The clean Qwen SFT example lives in `modal_apps/qwen_sft/`: `infra.py` deploys only the Qwen3 0.6B Megatron Bridge trainer plus SGLang rollout infra, and `sft.py` contains the single Tinker-style SFT flow. Deploy it with `GANKER_MODAL_GPU=A100 uv run modal deploy modal_apps/qwen_sft/infra.py`; run it with `GANKER_MODAL_GPU=A100 uv run modal run modal_apps/qwen_sft/sft.py --startup-timeout 900 --sglang-startup-timeout 900`.
