"""Run a clean tensor-parallel Qwen LoRA SFT smoke on Modal.
...
def main(
    ...
    if tensor_model_parallel_size <= 1:
        raise ValueError("tensor_model_parallel_size must be greater than 1 for this example")
    ...
    if world_size % tensor_model_parallel_size:
        raise ValueError("world size must be divisible by tensor_model_parallel_size")
    data_parallel_size = world_size // tensor_model_parallel_size
    effective_global_batch_size = global_batch_size or micro_batch_size * data_parallel_size

    run_id = comparison_id or f"qwen-tp-sft-{uuid.uuid4().hex[:8]}"
